...
,\end{equation}
where $\check{w}\ssl \ps=\check{w}\ssl \ps(\check{y}\ps)$ as the only part of $\tilde{w}\ps$ dependent on $\check{y}\ps$, and defining $y^{+}\equiv\left(k^{+}_x\right)^{-1 /3}\gls*{yck}$ in order to simplify or equations later.\glsadd{wck}
...
%				       &= \left(k_z\pw\right)^4\left(k_x\pw\right)^{-3}\left(\hat{P}\ww\pw\right)^2\int_{0}^{\infty}\frac{1}{2} \left| \dv{\check{u}\ww\pw}{\check{y}\pw}  \right| ^2 \dd{\check{y}\pw} \\
%				       &= \left(k_z\pw\right)^4\left(k_x\pw\right)^{-3}\hat{P}\ww\pw C_\text{min} \frac{\hat{W}\ps\ssl}{ik_z\pw \left(k_x\pw\right)^{-2 /3}}\int_{0}^{\infty}\frac{1}{2} \left| \dv{\check{u}\ww\pw}{\check{y}\pw}  \right| ^2 \dd{\check{y}\pw} \\
%				       &= \left(k_z\pw\right)^3\left(k_x\pw\right)^{-7 /3}\hat{P}\ww\pw C_\text{min} \hat{W}\ps\ssl\int_{0}^{\infty}\frac{1}{2} \left| \dv{\check{u}\ww\pw}{\check{y}\pw}  \right| ^2 \dd{\check{y}\pw} \label{eq:fiwwpwu}
%.\end{align}
%\begin{align}
